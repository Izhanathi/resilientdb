...
echo -e "// Number of worker threads at primary. " >> config.h
echo -e "#define THREAD_CNT 5 // This Should be the sum of following thread count + protocol specifig threads" >> config.h
echo -e "#define WORKER_THREAD_CNT 1" >> config.h
echo -e "#define BATCH_THREAD_CNT 2" >> config.h
echo -e "#define CHECKPOINT_THREAD_CNT 1" >> config.h
echo -e "#define EXECUTE_THREAD_CNT 1" >> config.h
echo -e "// IO THREADS" >> config.h
echo -e "#define REM_THREAD_CNT 2" >> config.h
...
echo -e "" >> config.h
echo -e "#define MESSAGE_PER_BUFFER 24" >> config.h
echo -e "" >> config.h
echo -e "// GeoBFT Setting " >> config.h
echo -e "#define GBFT false" >> config.h
echo -e "#define GBFT_CLUSTER_SIZE 4" >> config.h
echo -e "#define GBFT_CCM_THREAD_CNT 1" >> config.h
echo -e "" >> config.h
...
echo -e "#define CPU_FREQ 2.2" >> config.h
...
echo -e "#define STATS_DETAILED false" >> config.h
echo -e "#define STAT_BAND_WIDTH_ENABLE false" >> config.h
...
echo -e "#define TPORT_PORT 10000" >> config.h
echo -e "#define TPORT_WINDOW 20000" >> config.h
...
echo -e "#define PROG_TIMER 5 * BILLION" >> config.h
...
echo -e "#define WARMUP_TIMER  45 * BILLION" >> config.h
...
echo -e "#define TXN_PER_CHKPT 6 * BATCH_SIZE" >> config.h
...
echo -e "#define ENABLE_CHAIN false" >> config.h
...
echo -e "" >> config.h
echo -e "// To allow testing in-memory database or SQLite." >> config.h
echo -e "// Further, using SQLite a user can also choose to persist the data." >> config.h
